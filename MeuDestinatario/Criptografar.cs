...
namespace MeuDestinatario
{
    public class Criptografar
    {
        ...
        public static string CriptografaSenha(string senhaCripto, string chave)
        {
            ...
            {
                return string.Format("Digite os valores Corretamento : {0}", ex.Message);
            }
        }
        ...
        public static string DescriptografaSenha(string strCriptografada, string chave)
        {
            try
            {
                ...
                byte[] byteHash, byteBuff;
                string strTmpKey = chave;

                byteHash = objcriptoMd5.ComputeHash(ASCIIEncoding.ASCII.GetBytes(strTmpKey));
                objcriptoMd5 = null;
